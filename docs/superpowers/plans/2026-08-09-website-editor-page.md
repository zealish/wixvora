# Website Editor Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Port the standalone HTML website editor to a Next.js `/website-editor` page with 100% feature parity.

**Architecture:** Single client component file (`app/website-editor/page.tsx`) containing all React logic, inline sub-components, constants, types, and custom CSS. No database, no auth, no external dependencies beyond what already exists.

**Tech Stack:** Next.js 16.3.0, React 19.2.8, Tailwind CSS 4, next/font/google, TypeScript

## Global Constraints

- Route: `/website-editor` (guest-accessible, no auth)
- File: `app/website-editor/page.tsx` only (no extra components)
- `'use client'` directive required
- Google Fonts via `next/font/google` (Plus Jakarta Sans, Poppins, Playfair Display, Fira Code)
- 100% UI and feature parity with `/home/zealish/Downloads/wixvora_website_editor.html`
- No database integration
- No new npm dependencies
- Follow existing project conventions (Tailwind, TypeScript)

---

### Task 1: Scaffold page and font setup

**Files:**
- Create: `app/website-editor/page.tsx`

**Interfaces:**
- Produces: `WebsiteEditorPage` (default export), font CSS variable classes

- [ ] **Step 1: Create the page file with `'use client'` directive and font imports**
...
Run: `pnpm dev` and navigate to `http://localhost:3000/website-editor`
Expected: Page loads with "Website Editor - Loading..." text

- [ ] **Step 3: Commit**

```bash
git add app/website-editor/page.tsx
git commit -m "feat: scaffold website editor page with font setup"
```

---

### Task 2: Add custom CSS and type definitions

**Files:**
- Modify: `app/website-editor/page.tsx`

**Interfaces:**
- Consumes: Task 1 (page file exists)
- Produces: Types (Block, BlockType, PageSettings, etc.), custom CSS via `<style>` tag

- [ ] **Step 1: Add type definitions below the font imports**

Add these types after the font declarations and before the component:
...
Insert before the `export default function`:
...
const customStyles = `
  ...
  .block-outline { outline: 2px solid transparent; transition: all 0.15s ease-in-out; }
  .block-outline:hover { outline-color: #3b82f6; }
  .block-outline.is-selected { outline-color: #2563eb; outline-width: 2px; box-shadow: 0 0 0 4px rgba(37, 99, 235, 0.15); }
  .device-transition { transition: width 0.35s cubic-bezier(0.4, 0, 0.2, 1); }
  input[type="color"]::-webkit-color-swatch-wrapper { padding: 0; }
  input[type="color"]::-webkit-color-swatch { border: none; border-radius: 6px; }
  .editable-text-field {
    position: relative; outline: none; border-radius: 4px;
    transition: background-color 0.15s ease, box-shadow 0.15s ease;
  }
  .editable-text-field:hover:not([contenteditable="false"]) {
    background-color: rgba(37, 99, 235, 0.08);
    box-shadow: 0 0 0 1px rgba(37, 99, 235, 0.25); cursor: text;
  }
...
Update the component to render the style tag:

```typescript
export default function WebsiteEditorPage() {
  return (
    <div className={`${plusJakarta.variable} ${poppins.variable} ${playfair.variable} ${firaCode.variable} bg-slate-100 text-slate-800 font-sans antialiased overflow-hidden selection:bg-blue-600 selection:text-white`}>
...
Run: `pnpm dev` and navigate to `http://localhost:3000/website-editor`
Expected: Page loads, background is slate-100, no console errors

- [ ] **Step 4: Commit**

```bash
git add app/website-editor/page.tsx
git commit -m "feat: add type definitions and custom CSS for website editor"
```
...
**Interfaces:**
- Consumes: Task 2 (types exist)
- Produces: `COLOR_PALETTES`, `GRADIENT_PRESETS`, `BLOCK_CATALOG`, `PRESET_TEMPLATES`, `createUniqueId()`

- [ ] **Step 1: Add constants after the types**

Add this entire block before the component function:
    ...
    items: [
      {
        ...
        defaultProps: {
          ...
          columns: [
            ...
            { icon: 'code', title: 'Ekspor Bersih', desc: 'Dapatkan hasil kode HTML5 & Tailwind CSS murni kapan saja.', bgColor: '#f8fafc', textColor: '#0f172a', accentColor: '#d97706', btnText: 'Unduh Kode', btnUrl: '#' }
          ]
        }
      },
      {
...
```bash
git add app/website-editor/page.tsx
git commit -m "feat: add constants, block catalog, and utility functions"
```

---

### Task 4: Add Icon component

**Files:**
- Modify: `app/website-editor/page.tsx`

**Interfaces:**
- Consumes: Task 3 (constants exist)
- Produces: `Icon` inline function component

- [ ] **Step 1: Add the Icon component**

Insert before the `WebsiteEditorPage` function:
...
```bash
git add app/website-editor/page.tsx
git commit -m "feat: add Icon component with all SVG icons"
```

---

### Task 5: Add InlineText component
...
- Produces: `InlineText` inline function component

- [ ] **Step 1: Add the InlineText component**

Insert after the Icon component:
...
```bash
git add app/website-editor/page.tsx
git commit -m "feat: add InlineText component for inline editing"
```

---

### Task 6: Add main WebsiteEditorPage component with state management

**Files:**
- Modify: `app/website-editor/page.tsx`

**Interfaces:**
- Consumes: Tasks 2-5 (types, constants, Icon, InlineText)
- Produces: Full `WebsiteEditorPage` component with all state, handlers, and layout

- [ ] **Step 1: Replace the placeholder component with the full implementation**

This is the largest task. Replace the existing `WebsiteEditorPage` with the full component. The complete source is the HTML file ported to TSX. Key sections:

1. State declarations (useState hooks)
2. History management (undo/redo)
3. Block CRUD operations (add, delete, duplicate, move, toggle visibility)
4. Inline format commands
5. Export/Import functions
6. Render function with:
   - Top navbar (viewport switcher, actions)
   - Left sidebar (blocks, layers, templates, settings tabs)
   - Main canvas (blocks rendering with selection)
   - Right inspector panel (content, style, advanced tabs)
   - Export modal
   - Toast notifications

**Copy the entire HTML file content and convert:**
- `class` → `className`
- `onclick` → `onClick`
- `onchange` → `onChange`
- `for` → `htmlFor`
- Inline script logic → React state/handlers
- `ReactDOM.createRoot` → removed (Next.js handles this)

The full converted code should be approximately 1400-1600 lines.

- [ ] **Step 2: Verify the page renders correctly**

Run: `pnpm dev` and test all features:
- Page loads with navbar, hero, grid, footer
- Can add new blocks from left sidebar
- Can select blocks and edit properties in inspector
- Inline text editing works
- Color pickers update styles
- Undo/redo works
- Export HTML modal works
...
```bash
git add app/website-editor/page.tsx
git commit -m "feat: port full website editor from HTML to Next.js"
```
...
**Files:**
- Modify: `app/website-editor/page.tsx` (if needed)

**Interfaces:**
- Consumes: Task 6 (full component)
- Produces: Working website editor page
...
- [ ] **Step 2: Test all block types render**

Verify each block type:
- [ ] Navbar renders with logo, links, CTA
- [ ] Hero renders with badge, title, subtitle, buttons
- [ ] Container renders with content
- [ ] Grid Custom renders with columns
- [ ] Heading renders with correct tag
- [ ] Paragraph renders
- [ ] Image renders with caption
- [ ] Pricing renders with features
- [ ] Form Contact renders with input
- [ ] Footer renders with brand

- [ ] **Step 3: Test interactive features**

- [ ] Add each block type from catalog
- [ ] Duplicate a block
- [ ] Delete a block
- [ ] Move block up/down
- [ ] Toggle visibility
...
- [ ] Export HTML modal shows code
- [ ] Copy HTML to clipboard works

- [ ] **Step 4: Test inspector panel**

- [ ] Content tab: edit all text fields
- [ ] Style tab: change colors, gradients
- [ ] Advanced tab: add/remove grid columns
- [ ] Navbar links: add/remove
- [ ] Pricing features: add/remove

- [ ] **Step 5: Final commit if any fixes were made**

```bash
git add app/website-editor/page.tsx
git commit -m "fix: polish website editor page"
```
